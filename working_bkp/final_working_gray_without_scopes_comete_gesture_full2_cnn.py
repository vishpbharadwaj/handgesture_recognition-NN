...
# Fully connected layers
    ...
    with tf.name_scope('fc3') as scope:
        ...
        tf.summary.histogram("activations", com1_y_pred)
        print("\nshape of fc3: ")
    ...
    with tf.name_scope('final_softmax') as scope:
        W_fc4 = tf.Variable(tf.truncated_normal([com1_n_fc3, classes.size], stddev=0.1),name="w_fc4")
...
# Fully connected layers
    ...
    with tf.name_scope('fc3') as scope:
        ...
        tf.summary.histogram("weights", W_fc3)
        tf.summary.histogram("biases", b_fc3)
        tf.summary.histogram("activations", com2_y_pred)
        print("\nshape of fc3: ")
    ...
    #softmax layer:
    with tf.name_scope('final_softmax') as scope:
        W_fc4 = tf.Variable(tf.truncated_normal([com2_n_fc3, classes.size], stddev=0.1),name="w_fc4")
        b_fc4 = tf.Variable(tf.truncated_normal([6], stddev=0.1),name="b_fc3")
    ...
    #batch_xs, batch_ys = test_x[num_i,:,:,:], y_val[num_i,:]
    test_accuracy = sess.run(full_accuracy, feed_dict={x:test_x, y:y_val})
...
committee_full_gesture_cnn(alpha=learning_rate, keeprobs=keep_prob_fc_node)
