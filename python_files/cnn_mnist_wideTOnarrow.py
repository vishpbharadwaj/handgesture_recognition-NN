...
LOGDIR = "/home/vishnu/Dropbox/intel_works/ipython_notebooks/tf_tests/python_files/log_mnist_wideTOnarow"
...
########################################################################################
def mnist_wideTOnarrow(alpha):
    
    #tf.reset_default_graph()
    sess = tf.Session()

    mnist_img_size = 28 # 28*28*1
    
    x = tf.placeholder(tf.float32,[None,784])
    y = tf.placeholder(tf.float32,[None,10])

    reshape_list = tf.constant([None, mnist_img_size, mnist_img_size, 1])
    #x = tf.reshape(x,reshape_list) 
    x = tf.reshape(x,[-1,28,28,1]) 
    #y = tf.reshape(y,[None,10])
    
    #padding the input to get it to 32 * 32
    pad = 2
    x = tf.pad(x,[[0,0],[pad,pad],[pad,pad],[0,0]],"CONSTANT")

   
    """"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
#disabling conv1 layer as this is Lenet arch
...
#        tf.summary.image('conv1_kernal', grid, max_outputs=1)
    ...
    with tf.name_scope('softmax') as scope:
        W_fc3 = tf.Variable(tf.truncated_normal([n_fc2, 10], stddev=0.1),name="w_fc3")
        b_fc3 = tf.Variable(tf.truncated_normal([10], stddev=0.1),name="b_fc3")
    ...
    for i in range(epoch):
        for batch_i in range(mnist.train.num_examples // batch_size):
            if batch_i % 1 == 0:
                ...
                train_optimizer = sess.run(optimizer, feed_dict={x:batch_xs, y:batch_ys})
