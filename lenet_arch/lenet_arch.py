...
LOGDIR = "/home/vishnu/Documents/nn_files/tf_tests/lenet_arch/log_lenet_arch"


""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
########################################################################################
def lenet_arch_cnn(train_x, train_y, test_x, test_y, classes, alpha, keeprobs):
...
# Fully connected layers
    ...
    "---------------- reducing the learned parameters resolution ---------------- "
    decimal_round = 0

    com1_W_params["w1"] = tf_param_round(com1_W_params["w1"],decimal_round)
    com1_W_params["w2"] = tf_param_round(com1_W_params["w2"],decimal_round)
    com1_W_params["w3"] = tf_param_round(com1_W_params["w3"],decimal_round)

    
    com1_B_params["b1"] = tf_param_round(com1_B_params["b1"],decimal_round)
    com1_B_params["b2"] = tf_param_round(com1_B_params["b2"],decimal_round)
    com1_B_params["b3"] = tf_param_round(com1_B_params["b3"],decimal_round)

    W_fc1 = tf_param_round(W_fc1,decimal_round)
    b_fc1 = tf_param_round(b_fc1,decimal_round)

    W_fc2 = tf_param_round(W_fc2,decimal_round)
    b_fc2 = tf_param_round(b_fc2,decimal_round)

    W_fc3 = tf_param_round(W_fc3,decimal_round)
    b_fc3 = tf_param_round(b_fc3,decimal_round)

    print(">>>>>>>>>>>>>>>>>>>>>>>>>>> Rounded testing phase <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<")
    for batch_i in range(test_x.shape[0] // batch_size_test):
        batch_xs, batch_ys = test_x[batch_i*batch_size_test:batch_size_test+batch_i*batch_size_test,:,:,:], y_val[batch_i*batch_size_test:batch_size_test+batch_i*batch_size_test ,:]
        # test accuracy
        test_accuracy = sess.run(com1_accuracy, feed_dict={x:batch_xs, y:batch_ys})
        print("batch %d/%d"%(batch_i, test_x.shape[0] // batch_size_test))
        print("round accuracy: %f"%test_accuracy)
        print("\n")
    "---------------------------------------------------------------------------- "
...
train_x, train_y, test_x, test_y  = load_mnist_unnormalized()
classes = 10
...
"padding zeros to make it to 32x32 from 28x28"
...
print("No. of gestures: %d"%classes)
...
com1_Fsize_params["fsize1"] = 5 # filter size of n*n for convolution
com1_Fnum_params["fnum1"] = 6 # No. of filters required
com1_Stride_params["stride1"] = [1,1,1,1]
com1_Maxpool_ops["pool1"] = True #change here if pooling not required
...
com1_fc_shape_last_cnn = 400 #"this is done because tf.reshape had some issues"
...
w_cnn, b_cnn, w_fc1, b_fc1, w_fc2, b_fc2, w_fc3, b_fc3 = lenet_arch_cnn(train_x, y_org, test_x, y_val, classes, alpha=learning_rate, keeprobs=keep_prob_fc_node)
